...
with st.sidebar.form("add_form", clear_on_submit=True):
    ...
    new_year = st.number_input("Year", min_value=0, max_value=2024, value=2024)
    ...
    if submit_button:
        ...
        st.session_state.gallery_data = pd.concat([st.session_state.gallery_data, pd.DataFrame([new_row])], ignore_index=True)
...
filtered_df = st.session_state.gallery_data[st.session_state.gallery_data['Artist'].str.contains(search_query, case=False)]
...
with col1:
    ...
    fig_bar = px.bar(filtered_df, x='Title', y='Price', color='Artist',
                     title="Artwork Prices Comparison", template="plotly_white")
    st.plotly_chart(fig_bar, use_container_width=True)

with col2:
    st.subheader("⏳ Distribution by Period")
    fig_pie = px.pie(filtered_df, names='Period', title="Works by Period",
